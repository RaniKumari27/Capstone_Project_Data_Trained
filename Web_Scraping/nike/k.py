...
def nike_scrapper(url, gender, csv_file_name):
    ...
    close_button = driver.find_element(By.XPATH, '//a[@title="United States"]')
    close_button.click()
    time.sleep(2)
    ...
    while True:
        ...
        try:
            close_button1 = driver.find_element(By.XPATH,
                                                '//button[@class="pre-modal-btn-close bg-transparent"]')
            close_button1.click()
        except:
            pass
    ...
    total_products = driver.find_element(By.XPATH,
                                         '//span[@class="wall-header__item_count"]').get_attribute('textContent')
    ...
    for url in urls:
        ...
        try:
            ...
            category = driver.find_element(By.XPATH,
                                           '//h2[@class="headline-5-small pb1-sm d-sm-ib css-1ppcdci"]').get_attribute('textContent')
            price = driver.find_element(By.XPATH,
                                        '//div[@class="product-price is--current-price css-1emn094"]').get_attribute('textContent')
            price = int(re.findall('\d+', price)[0])
            description = driver.find_element(By.XPATH,
                                              '//div[@class="description-preview body-2 css-1pbvugb"]/p').get_attribute('textContent')
            try:
                description_long = driver.find_element(By.XPATH,
                                                       '//div[@class="pi-pdpmainbody"]').get_attribute('textContent')
            except:
                try:
                    description_long = driver.find_element(By.XPATH,
                                                           '//div[@class="nby-product-desc-container"]').get_attribute('textContent')
                except:
                    description_long = ""
        except:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        slider_data = [slider.get_attribute(
                            'style') for slider in sliders]
                        slider_data = [float(re.search(
                            'margin-left: calc\((.+)% - 5px\);', slider).group(1)) for slider in slider_data]
                    except:
                        size = ""
                        comfort = ""
                        durability = ""
                    ...
                    n_reviews = driver.find_element(By.XPATH,
                                                    '//div[@class="TTreviewCount"]').get_attribute('textContent')
                    n_reviews = float(
                        (re.findall(("\d+.\d+|\d+"), n_reviews)[0]).replace(",", ""))
                    ...
                    for count, review in enumerate(reviews):
                        ...
                        r_title = review.find_element(By.XPATH,
                                                      './/div[@class="TTreviewTitle"]').get_attribute('textContent')
                        r_raiting = ""
                        r_body = review.find_element(By.XPATH,
                                                     './/div[@class="TTreviewBody"]').get_attribute('textContent')
                        r_date = review.find_element(By.XPATH,
                                                     './/div[@itemprop="dateCreated"]').get_attribute('datetime')
                        r_date = ""
...
nike_scrapper("https://www.nike.com/w/womens-shoes-5e1x6zy7ok?sort=newest",
              "woman", "nike_shoes_woman.csv")
